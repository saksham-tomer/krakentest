export const processes = [
  {
    id: 1,
    backgroundColor: "#CDBCFF",
    imgSrc: "/img/process/process1.svg",
    alt: "process",
    title: "1. Set up your wallet",
    description:
      "Once you've set up your wallet of choice, connect it to OpenSeaby clicking the NFT Marketplace in the top right corner.",
  },
  {
    id: 2,
    backgroundColor: "#C4F2E3",
    imgSrc: "/img/process/process2.svg",
    alt: "process",
    title: "2. Create Your Collection",
    description:
      "Click Create and set up your collection. Add social links, a description, profile & banner images, and set a secondary sales fee.",
  },
  {
    id: 3,
    backgroundColor: "#CDDFFB",
    imgSrc: "/img/process/process3.svg",
    alt: "process",
    title: "3. Add Your NFTs",
    description:
      "Upload your work (image, video, audio, or 3D art), add a title and description, and customize your NFTs with properties, stats.",
  },
  {
    id: 4,
    backgroundColor: "#FFD0D0",
    imgSrc: "/img/process/process4.svg",
    alt: "process",
    title: "4. List Them For Sale",
    description:
      "Choose between auctions, fixed-price listings, and declining-price listings. You choose how you want to sell your NFTs!",
  },
];
export const process2 = [
  {
    stepNumber: "01",
    title: "Create an account",
    description:
      "Once you've set up your wallet of choice, connect it to OpenSeaby clicking the NFT Marketplacein.",
    svgPath:
      "M14.256 21.744L12 24l-2.256-2.256C5.31 20.72 2 16.744 2 12 2 6.48 6.48 2 12 2s10 4.48 10 10c0 4.744-3.31 8.72-7.744 9.744zm-8.233-6.328C7.491 17.606 9.695 19 12.16 19c2.464 0 4.669-1.393 6.136-3.584A8.968 8.968 0 0 0 12.16 13a8.968 8.968 0 0 0-6.137 2.416zM12 11a3 3 0 1 0 0-6 3 3 0 0 0 0 6z",
  },
  {
    stepNumber: "02",
    title: "Link your bank account",
    description:
      "Once you've set up your wallet of choice, connect it to OpenSeaby clicking the NFT Marketplacein.",
    svgPath:
      "M2 20h20v2H2v-2zm2-8h2v7H4v-7zm5 0h2v7H9v-7zm4 0h2v7h-2v-7zm5 0h2v7h-2v-7zM2 7l10-5 10 5v4H2V7zm10 1a1 1 0 1 0 0-2 1 1 0 0 0 0 2z",
  },
  {
    stepNumber: "03",
    title: "Start buying & selling",
    description:
      "Once you've set up your wallet of choice, connect it to OpenSeaby clicking the NFT Marketplacein.",
    svgPath:
      "M9.33 11.5h2.17A4.5 4.5 0 0 1 16 16H8.999L9 17h8v-1a5.578 5.578 0 0 0-.886-3H19a5 5 0 0 1 4.516 2.851C21.151 18.972 17.322 21 13 21c-2.761 0-5.1-.59-7-1.625L6 10.071A6.967 6.967 0 0 1 9.33 11.5zM5 19a1 1 0 0 1-1 1H2a1 1 0 0 1-1-1v-9a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v9zM18 5a3 3 0 1 1 0 6 3 3 0 0 1 0-6zm-7-3a3 3 0 1 1 0 6 3 3 0 0 1 0-6z",
  },
];
export const process3 = [
  {
    id: 1,
    backgroundColor: "#CDBCFF",
    iconSrc: "/img/process/process7.svg",
    alt: "process",
    title: "1. Prepare your game story",
    bgClass: "bg-accent",
    description:
      "The AI game engine can understand your game world and story. It will adapt to its tone and expand it!",
  },
  {
    id: 2,
    backgroundColor: "#C4F2E3",
    iconSrc: "/img/process/process6.svg",
    alt: "process",
    title: "2. Create your game and upload your story",
    bgClass: "bg-green",
    description:
      "This is where you can enhance the game experience for your players by adding key locations and Npc personality images, so that the game engine can generate natural reactions on the fly inside the game.",
  },
  // {
  //   id: 3,
  //   backgroundColor: "#CDDFFB",
  //   iconSrc: "/img/process/process7.svg",
  //   alt: "process",
  //   title: "3. Add Your NFTs",
  //   bgClass: "bg-blue",
  //   description:
  //     "Upload your work (image, video, audio, or 3D art), add a title and description, and customize your NFTs with properties, stats.",
  // },
  {
    id: 3,
    backgroundColor: "#FFD0D0",
    iconSrc: "/img/process/process8.svg",
    alt: "process",
    title: "3. Publish your game and monetize",
    bgClass: "bg-red",
    description:
      "you can publish games for free or for a fee. Congrats your first Ai Game is out!",
  },
];
