...
export default function Header({ sidebarOpen, setSidebarOpen }) {
  ...
  return (
    <>
      <header
        className={`js-page-header w-full fixed top-0 right-0 z-20 ${
          sidebarOpen ? "w-[calc(100%-200px)] md:w-content-with-sidebar" : ""
        } lg:w-content-with-sidebar backdrop-blur transition-colors  ${
      ...
      >
        <div className="flex items-center px-6 py-6 xl:px-24">
          <button onClick={() => setSidebarOpen(!sidebarOpen)}>
            {sidebarOpen ? (
              <FiX size={24} className="lg:hidden" />
            ) : (
              <FiMenu size={24} className="lg:hidden" />
            )}
          </button>
          {/* Menu / Actions */}
