...
export default function Header() {
  ...
  const { gamePointCoupons } = useUser();
  ...
  return (
    <>
      ...
      >
        <div className="flex items-center px-6 py-6 xl:px-12 2xl:px-24">
          {/* Logo */}
          <Link href="/" className="shrink-0 flex items-center gap-2">
            ...
            <Image
              ...
              alt="Kraken | Game Marketplace"
            />
            <p className="text-[#FA8704] text-2xl font-bold">Kraken</p>
          </Link>

          {/* Search */}
          {/* <Link
            href={`/create`}
            className="ml-auto hidden sm:block h-[38px] animate-gradient--no-text-fill block animate-gradient overflow-hidden rounded-xl !bg-clip-border text-center shadow-md transition-shadow hover:shadow-lg"
          >
            <button className="bg-black/[.3] flex items-center justify-center bg-background px-3 py-2 font-bold text-md text-white transition-all duration-200 ease-in-out transform hover:scale-105">
              Create Game
            </button>
          </Link> */}
          ...
          {/* Menu / Actions */}
          <div className="js-mobile-menu invisible lg:visible fixed inset-0 z-10 items-center bg-white opacity-0 dark:bg-jacarta-800 lg:relative lg:inset-auto lg:flex lg:bg-transparent lg:opacity-100 dark:lg:bg-transparent ">
            ...
            <Link
              href="/wallet"
              className="js-nav-dropdown group-dropdown relative mx-2 flex h-10 w-10 items-center justify-center rounded-full border border-jacarta-100 bg-white transition-colors hover:border-transparent hover:bg-accent focus:border-transparent focus:bg-accent dark:border-transparent dark:bg-white/[.15] dark:hover:bg-accent"
            >
              {gamePointCoupons?.some(coupon => {
                const today = new Date();
                return coupon.expiration_date && new Date(coupon.expiration_date) >= today && !coupon.redeemed;
              }) && (
                <span className="absolute -top-1 -right-1 h-5 w-5 rounded-full bg-accent text-xs text-white flex items-center justify-center">
                  {gamePointCoupons.filter(coupon => 
              ...
              <svg
                ...
                width="24" 
                height="24"
                className="h-5 w-5 fill-jacarta-700 transition-colors group-hover:fill-white dark:fill-white"
              >
          ...
          {/* Mobile Menu Actions */}
          <div className="ml-auto sm:ml-0 flex lg:hidden">
            {/* Profile */}
            {user ? (
              <Link
                href="/profile"
